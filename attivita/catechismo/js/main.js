// --- Aggiunta locandine ---
// Sidebar locandine disattivata su richiesta (teniamo solo eventi e documenti):
// gli elementi non esistono più nella pagina, quindi guardiamo prima di usarli
// per non rompere il resto dello script.
        ...
        for (let _loc of locandineDb) {
            const locHTML = `
                <div class="locandina-item">
                    <img src="/attivita/catechismo/img/locandine/${_loc.nome}" alt="Locandina ${_loc.data}">
                </div>
            `;
...
// --- Aggiunta Iscrizioni ---
// Card disattivata su richiesta: gli elementi non esistono più nella pagina,
// quindi questo blocco viene saltato per non rompere lo script.
const iscrizioniList = document.getElementById('iscrizioni-list');
const iscrizioniCard = document.getElementById('card-iscrizioni');

if (iscrizioniList && iscrizioniCard) {
...
// Card disattivata su richiesta: guardia aggiunta per lo stesso motivo di sopra.
...
if (toggleIscrizioni && menuIscrizioni && arrowIcon) {
...
// Sidebar locandine disattivata su richiesta: guardia aggiunta per non rompere
// lo script quando #locandineGrid non è presente nella pagina.
if (gridLocandine) {
    const itemsPerPage = 4;
    let currentPage = 1;

    const grid = gridLocandine;
    ...
    if (btnPrev && btnNext && paginationControls) {
